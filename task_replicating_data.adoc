...
版本需求:: 在複寫資料之前、您應該先確認來源和目的地磁碟區是否執行相容 ONTAP 的功能性更新。如需詳細資訊、請參閱 http://["資料保護電源指南"^]。
...
若要執行反向重新同步、將目的地磁碟區的資料重新同步至來源磁碟區、請參閱 http://["《》《 9 Volume Disaster Recovery Express 指南》 ONTAP"^]。
