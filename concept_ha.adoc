...
NOTE: Cloud Manager 會建立一個 https://["AWS 分散配置群組"^] 然後啟動該放置群組中的兩個 HA 節點。放置群組可將執行個體分散到不同的基礎硬體、藉此降低同時發生故障的風險。此功能可從運算角度而非磁碟故障角度改善備援。
...
您可以從Cloud Manager輕鬆識別正確的IP位址。下圖顯示儲存系統檢視：

image:screenshot_mount.gif["螢幕擷取畫面：顯示選取 Volume 時可用的 Mount Command 。"]

下圖顯示Volume View：

image:screenshot_mount_volume_view.gif["螢幕擷取畫面：顯示Volume的功能表選項、其中包括Mount（掛載）選項。"]
