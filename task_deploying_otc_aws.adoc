...
| 新增標記 | AWS 標籤是 AWS 資源的中繼資料。Cloud Manager 會將標記新增至 Cloud Volumes ONTAP 該執行個體、以及與該執行個體相關聯的每個 AWS 資源。建立工作環境時、您最多可以從使用者介面新增四個標記、然後在建立之後新增更多標記。請注意、在建立工作環境時、 API 不會限制您使用四個標記。如需標記的相關資訊、請參閱 https://["AWS 文件：標記 Amazon EC2 資源"^]。 
...
+
如果您偏好使用自己的原則、就必須符合 http://["有關節點的原則要求 Cloud Volumes ONTAP"^]。
...
| 新增標記 | AWS 標籤是 AWS 資源的中繼資料。Cloud Manager 會將標記新增至 Cloud Volumes ONTAP 該執行個體、以及與該執行個體相關聯的每個 AWS 資源。如需標記的相關資訊、請參閱 https://["AWS 文件：標記 Amazon EC2 資源"^]。 
...
. 在「IAM角色」頁面上、您應該保留預設選項、讓Cloud Manager為您建立角色。
+
如果您偏好使用自己的原則、就必須符合 http://["有關節點和 HA 中介器的原則要求 Cloud Volumes ONTAP"^]。
