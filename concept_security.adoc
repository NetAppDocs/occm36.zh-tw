...
支援NetApp Volume Encryption搭配外部金鑰管理伺服器。Cloud Volumes ONTAP不支援Onboard Key Manager。您可以在中找到支援的金鑰管理程式 http://["NetApp 互通性對照表工具"^] 在*關鍵經理*解決方案下。
...
當您在 Cloud Volumes ONTAP AWS 中啟動一個支援功能系統時、可以使用啟用資料加密 http://["AWS 金鑰管理服務（ KMS ）"^]。Cloud Manager 會使用客戶主金鑰（ CMK ）要求資料金鑰。
...
https://["Azure 儲存服務加密"^] Azure 中 Cloud Volumes ONTAP 預設會啟用靜止資料的功能、以供資料使用。無需設定。
